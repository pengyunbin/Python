...
def compute_pageranks(google_mat, tol=1e-8):
    ...
    while rel_change > tol:
        ...
        new_dist = ((dist.T).dot(google_mat)).T
...
def main():
    ...
    adj_mat_norm[dangling == 0] = (adj_mat[dangling == 0].T / out_links[dangling == 0]).T
    ...
    pageranks_eig = pageranks_eig / np.sum(pageranks_eig)
