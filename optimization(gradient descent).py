...
def gradient_descent(func_grad, init_point, step_size, num_iters):
    ...
    for i in range(num_iters):
        ...
        scaled_gradient = [(curr_gradient[i] * step_size) for i in range(len(curr_gradient))]

        # subtract scaled gradient from current iterate
        curr_iter = [(curr_iter[i] - scaled_gradient[i]) for i in range(len(curr_iter))]
...
def loss_grad_calculator(X, Y, scalar_loss_deriv, beta):
    ...
    for i in range(n):
        ...
        loss_grad_on_i = [scalar_loss_deriv(inner_prod, Y[i])*(X[i][j]) for j in range(len(X[i]))]

        # add loss_grad_on_i (elementwise) to the sum so far
        loss_grad_sum = [(loss_grad_sum[x] + loss_grad_on_i[x]) for x in range(len(loss_grad_on_i))]
...
def loss_grad_1d_calculator(X, Y, scalar_loss_deriv, beta, j):
    ...
    for i in range(n):
        ...
        # calculate partial derivative (w.r.t. beta_j) of the loss on example i and add it to the sum so far
...
def main():
    ...
    # using the loss and loss_deriv dictionaries 
    # along with the 3 functions: loss_calculator, loss_grad_calculator,
    # and loss_grad_1d_calculator
    squared_loss = lambda beta: loss_calculator(X, Y, loss['squared'], beta)
    squared_loss_grad = lambda beta: loss_grad_calculator(X, Y, loss_deriv['squared'], beta)
    squared_loss_grad_1d = lambda beta, j: loss_grad_1d_calculator(X, Y, loss_deriv['squared'], beta, j)
    ...
    # along with the 3 functions: loss_calculator, loss_grad_calculator,
    # and loss_grad_1d_calculator
    logistic_loss = lambda beta: loss_calculator(X, Y, loss['logistic'], beta)
    logistic_loss_grad = lambda beta: loss_grad_calculator(X, Y, loss_deriv['logistic'], beta)
    logistic_loss_grad_1d = lambda beta, j: loss_grad_1d_calculator(X, Y, loss_deriv['logistic'], beta, j)
